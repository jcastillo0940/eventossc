...
@section('content')
    <div class="mb-12 flex items-center justify-between">
        <div>
            ...
            <p class="text-slate-500 font-medium italic italic text-sm">Actualice los datos del patrocinador y sus eventos vinculados.</p>
        </div>
        ...
        <div class="dashboard-card p-10 space-y-8">
            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    @php $selectedEvents = $brand->events->pluck('id')->toArray(); @endphp
                    @foreach($events as $event)
                        <label class="flex items-center gap-3 p-3 bg-white border border-slate-100 rounded-xl cursor-pointer hover:border-indigo-500 transition-all">
                            <input type="checkbox" name="event_ids[]" value="{{ $event->id }}" 
                                   {{ in_array($event->id, old('event_ids', $selectedEvents)) ? 'checked' : '' }}
                ...
                <div class="space-y-3">
                    ...
                    @if($brand->logo_path)
                        <div class="mt-2 flex items-center gap-3">
                            <div class="p-1 bg-slate-50 border border-slate-100 rounded-lg">
                                <img src="{{ $brand->logo_path }}" class="h-8 w-auto object-contain">
                            </div>
                            <span class="text-[9px] font-black text-indigo-500 uppercase italic italic">Logo actual</span>
                        </div>
                    @endif
                </div>
            </div>
