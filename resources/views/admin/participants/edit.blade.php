...
@section('header_title', 'Gestionar Competidor')

@section('content')
    <div class="mb-12 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-black text-slate-900 uppercase italic italic">Editar <span class="text-emerald-500">Participante</span></h1>
            <p class="text-slate-500 font-medium italic italic">Actualice los datos del competidor registrado.</p>
        </div>
    ...
    <form action="{{ route('admin.participants.update', $participant) }}" method="POST" enctype="multipart/form-data" class="space-y-10 max-w-2xl mx-auto">
        @csrf
        @method('PUT')
        ...
        <div class="dashboard-card p-10 space-y-8">
            <div class="space-y-2">
                ...
                <input type="text" name="name" required value="{{ old('name', $participant->name) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="category" required value="{{ old('category', $participant->category) }}"
                           class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all italic italic font-medium">
                </div>
            </div>
            ...
            <div class="flex items-center justify-between p-6 rounded-2xl bg-slate-50/50 border border-slate-100">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $participant->is_active ? 'checked' : '' }}>
